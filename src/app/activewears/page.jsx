...
export default function ActivewearsPage() {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        const res = await fetch('/api/getproducts/activewears')
        const json = await res.json()
        if (!json.success) throw new Error(json.error || 'Failed to fetch activewears')
        setProducts(json.data || [])
      } catch (err) {
        console.error('❌ Error:', err)
        setError('Failed to load activewears.')
      } finally {
        setLoading(false)
      }
    }
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-[#1f1f1f] to-[#121212] text-white overflow-hidden px-6 py-20">
      ...
      <div className="relative z-10 text-center mt-8">
        <h1 className="text-4xl font-semibold text-white tracking-wide">
          Browse <span className="text-yellow-400">Activewears</span>
        </h1>
      </div>
      ...
      <div className="relative z-10 mt-8 text-center">
        ...
        {!loading && !error && products.length === 0 && (
          <p className="text-gray-400">No activewears available.</p>
        )}
      </div>
