  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        ...
        setError('Failed to fetch jeans')
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-[#1f1f1f] to-[#121212] overflow-hidden">
      ...
      <div className="relative z-10 text-center mt-24">
        ...
        <p className="mt-2 text-gray-400">Denim that defines your edge</p>
      </div>

      {/* Status Messages - identical to home */}
      <div className="relative z-10 mt-10 px-6 text-center">
        {loading && <p className="text-gray-500">Loading jeans...</p>}
        {error && <p className="text-red-500">{error}</p>}
        {!loading && products.length === 0 && !error && (
          <p className="text-gray-500">No jeans available at the moment.</p>
        )}
      </div>
