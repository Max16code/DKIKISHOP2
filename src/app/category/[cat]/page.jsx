...
export default function CategoryPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Request up to 150 products (API enforces the limit)
        const res = await fetch(`/api/getproducts/${cat}?available=true&limit=150`);
        ...
        // Additional client-side filter (though API already filters available & in-stock)
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-[#1f1f1f] to-[#121212] overflow-hidden">
      ...
      <div className="relative z-10 text-center mt-24">
        <h1 className="text-4xl md:text-5xl font-semibold text-white tracking-wide">
          Browse <span className="text-yellow-400">{categoryName}</span>
        </h1>
              ...
              return (
                <Link href={`/product/${product._id}`} key={product._id || index} passHref>
                  <motion.div
                    ...
                    className={`flex flex-col cursor-pointer shadow-md hover:shadow-yellow-500/20 transition-shadow duration-300 relative rounded-lg overflow-hidden ${
                      !inStock ? 'opacity-70' : ''
                    }`}
                  >
          ...
          <div className="relative z-10 mt-12 flex justify-center gap-6 pb-20">
            ...
            {hasMore && (
              <button
                ...
                className={`px-10 py-3 bg-yellow-500 hover:bg-yellow-600 text-black font-bold rounded-full shadow-lg transition-all duration-300 transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
