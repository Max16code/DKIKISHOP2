...
export default function AccessoriesPage() {
  ...
  useEffect(() => {
    const getProducts = async () => {
      try {
        // Fetch accessories only (adjust endpoint/query as needed)
        const res = await fetch('/api/getproducts?category=accessories&available=true')
        const data = await res.json()

        if (!Array.isArray(data)) throw new Error('Invalid product format')

        // Client-side availability filter (backup)
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-[#1f1f1f] to-[#121212] overflow-hidden">
      {/* Faint Logo Background - same as home */}
      ...
      {/* Category Title - styled like home welcome */}
      <div className="relative z-10 text-center mt-24">
        <h1 className="text-4xl md:text-5xl font-semibold text-white tracking-wide">
          Accessories
        </h1>
        <p className="mt-2 text-gray-400">Complete your look with timeless pieces</p>
      </div>

      {/* Status Messages - same as home */}
      ...
      {/* Product Grid - identical structure & style to home */}
